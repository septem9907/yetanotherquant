# Simulates a basket of 10 correlated binary trades using a Gaussian copula.
# Each trade is binary: return = RET_UP (+13.4%) or RET_DN (-5.15%) with equal probability.
# Copula correlation = 0.31 between all pairs; overall portfolio return = equal-weight average.
# The `returns` vector is consumed by 5_6b.r (now self-contained there).
...
CORR   = 0.31
RET_UP = (22.00 - 19.40) / 19.40   # +13.4% (SLV target price scenario)
RET_DN = (18.40 - 19.40) / 19.40   # -5.15%

# Generate correlated uniform marginals via a 10-dim Gaussian copula
tmp <- normalCopula(CORR, dim=10)
x   <- rcopula(tmp, N_SIMULATIONS)

# Map uniforms to binary outcomes (0/1) via binomial quantile
b1  = qbinom(x[,1],  1, 0.5)
b2  = qbinom(x[,2],  1, 0.5)
b3  = qbinom(x[,3],  1, 0.5)
b4  = qbinom(x[,4],  1, 0.5)
...
print(round(cor(mRets), 2))   # verify pairwise correlations are ~0.31
