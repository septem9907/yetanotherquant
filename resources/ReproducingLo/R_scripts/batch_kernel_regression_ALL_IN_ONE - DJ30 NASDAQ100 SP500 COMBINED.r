...
#Dow Jones Industrial Avarege (DJ30)
#NASDAQ100
#SAP500
#
#The bandwidthFactor is always 1  
#to reduce computational time (about 3 hours on 2.80Hz processor).

#Start looking from the end of the script to adjust paths of input .xls Files
#Look at the definition of "filename" to set filepath for graphical output.
...
channel <- odbcConnectExcel("D:\\BOOK\\WEBSITE\\ReproducingLo\\daily_data\\DJ30.xls")
...
channel <- odbcConnectExcel("D:\\BOOK\\WEBSITE\\ReproducingLo\\daily_data\\NASDAQ100.xls")
marketData <- sqlFetch(channel,"NASDAQ100")
odbcClose(channel)
attach(marketData)
findPattern("TBOT")
findPattern("TTOP")
...
channel <- odbcConnectExcel("D:\\BOOK\\WEBSITE\\ReproducingLo\\daily_data\\SP500.xls")
marketData <- sqlFetch(channel,"SP500")
odbcClose(channel)
attach(marketData)
findPattern("TBOT")
findPattern("TTOP")
