...
from yetanotherquant.chapter6.options import (
    bs_call_price, bs_greeks, option_scenario_sweep, plot_greeks_comparison,
)
...
print(f"  At S0 (i=0), T=4: price={df_now.filter(df_now['price_change_cents']==0)['value'][0]:.6f}")
print(f"  At S0 (i=0), T=3: price={df_later.filter(df_later['price_change_cents']==0)['value'][0]:.6f}")
