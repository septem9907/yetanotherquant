...
from yetanotherquant.chapter5.copula import (
    gaussian_copula_binary, portfolio_returns_from_copula,
...
asset_rets = gaussian_copula_binary(
    corr=CORR, n_assets=N_ASSETS,
    ret_up=RET_UP, ret_dn=RET_DN,
    p_up=0.5, n_sim=100_000, seed=42,
)
...
axes[0].plot(np.linspace(lgr.min(), lgr.max(), 300),
...
axes[1].plot(np.linspace(mdd.min(), mdd.max(), 300),
...
clayton_rets = clayton_copula_normal_margins(mean_57, sigma_57, theta=2.0, n_sim=10_000, seed=42)
