...
from yetanotherquant.chapter5.portfolio import (
    correlation_matrix, estimate_sigma, nekrasov_optimal, implied_drift,
)
...
cov_52 = np.array([[s1**2,          rho*s1*s2],
                   [rho*s1*s2,      s2**2]])
...
print(f"  (With rho=0.5, symmetric mu/sigma, expect equal fractions)")
...
r_f_a  = 0.01
cov_52a = np.array([[s1_a**2,           rho_a*s1_a*s2_a],
                    [rho_a*s1_a*s2_a,   s2_a**2]])
...
vola_meo = 0.01883767
res_54 = implied_drift(29.79, 35.40, 26.40, 0.5, 0.5, 141, vola_meo,
                       n_steps=50, n_sim=20_000, seed=42)
...
if "MEO.DE" in tickers_53 and "SZU.DE" in tickers_53:
    ...
    cov_2    = np.array([[vola_meo**2,            vola_meo*vola_szu*rho_ms],
