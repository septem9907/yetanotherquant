...
from yetanotherquant.chapter3.kelly import (
    analytical_kelly_coin,
    backtest_kelly_fractions,
    drawdown_risk_at_fraction,
...
ax2.plot(
    strategies["kelly"], linewidth=2, label=f"Kelly ({f_star:.0%})", color="darkblue"
)
ax2.plot(
    strategies["half_kelly"],
    linewidth=2,
    label=f"Half-Kelly ({f_star/2:.0%})",
    color="grey",
    linestyle="--",
)
...
mlw = kelly_simulation(
    mu_m, sigma_m, r_monthly, n_steps=100, n_sim=5_000, n_periods=120, seed=42
)
...
sensitivity = kelly_sensitivity(
    m_rets, r_monthly, n_iterations=3, n_steps=100, n_sim=2_000, n_months=120, seed=42
)
for row in sensitivity:
    print(
        f"  Iter {row['iteration']}: mu_est={row['mu_est']:.4f}  "
...
for mu_k, sigma_k, kf, label in [(0.0003, 0.02, 0.76, "A"), (0.0006, 0.04, 0.65, "B")]:
    ...
    print(
        f"  Stock {label} (f={kf}): P(MDD<-5%)={res['prob_mdd_lt_5pct']:.3f}  "
