...
def run() -> bool:
    ...
    all_pass &= check(
        "Analytical Kelly fraction (coin-toss)", py_fstar, R_KELLY_FRAC, 5e-3
    )
    ...
    all_pass &= check(
        "Expected growth rate at f*", py_rate_at_fstar, R_KELLY_RATE, 1e-9
    )

    # 3. Growth rate at 100% bet is negative
    py_full_rate = float(py_rates[-1])
    all_pass &= check(
        "Expected growth rate at f=100% (should be negative)",
        py_full_rate,
        R_FULL_RATE,
        1e-9,
    )
    ...
    all_pass &= check(
        "Simulated Kelly fraction (coin-toss, stochastic)",
        sim_fstar,
        R_KELLY_FRAC,
        tol_sim,
    )
