...
from yetanotherquant.chapter5.copula import gaussian_copula_binary, sample_clayton_copula
...
def run() -> bool:
    ...
    all_pass &= check("Sigma[0,0] matches analytical formula",
                      float(Sigma_est[0, 0]), expected_Sigma_11, tol=5e-4)
    ...
    all_pass &= check("Single-asset Nekrasov fraction matches formula",
                      float(u[0]), expected_nekrasov, tol=0.05)
    ...
    all_pass &= check("Symmetric 2-asset: f1 ≈ f2",
                      float(u2[0]), float(u2[1]), tol=0.01)
    ...
    rets = gaussian_copula_binary(
        corr=target_corr, n_assets=n_assets,
        ret_up=0.134, ret_dn=-0.0515,
        p_up=0.5, n_sim=50_000, seed=42,
    )
    ...
    all_pass &= check("Gaussian copula: mean pairwise corr is positive and < target",
                      mean_corr, target_corr, tol=target_corr)  # within ±target_corr
    ...
    all_pass &= check("Clayton copula Kendall's tau ≈ theta/(theta+2) = 0.5",
                      float(kt), expected_tau, tol=0.03)
